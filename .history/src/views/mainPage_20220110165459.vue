<template>
    <div class="backgroundDiv vw-100">
        <div class="backImg">
            <div class="pt-10 mx-auto">
                <h1 class="text-4xl font-normal">Images of World Heritage</h1>
            </div>
            <div>
                <button @click="data">oo</button>
            </div>
            <!-- <div class="Homebtn">
                <router-link to="/">Home</router-link>
            </div> -->
            <div class="imgNums mt-40 lg:mt-10 lg:ml-64">
                <h1 class="text-2xl font-bold">Number:{{ number }}</h1>
            </div>
            <div class="imgDataDisplay mt-5 bg-gray-800 w-32 h-20">
                <!-- <h4 class="pt-2 text-white">{{ names }}</h4> -->
                <h4 class="pt-8 pr-20 text-white">by:{{ country }}</h4>
            </div>
            <div class="btnBackDisplay bg-gray-400 h-24">
                <!-- <input v-model="number" /> -->
                <button @click="ones()" v-bind:value="one">1</button>
                <button two" @click="nums(this.id)" value="2">2</button>
                <button three" @click="nums(this.id)" value="3">3</button>
                <button four" @click="nums(this.id)" value="4">4</button>
                <button five" @click="nums(this.id)" value="5">5</button>
                <button six" @click="nums(this.id)" value="6">6</button>
                <button seven" @click="nums(this.id)" value="7">7</button>
                <button eight" @click="nums(this.id)" value="8">8</button>
                <button nine" @click="nums(this.id)" value="9">9</button>
                <button zero" @click="nums(this.id)" value="0">0</button>
                <button @click="result">result</button>
            </div>
            <button
                class="text-white text-2xl rounded-full download bg-gray-400 h-20"
            >
                Download
            </button>
        </div>
    </div>
</template>

<script lang="ts">
import { defineComponent } from "vue";
import imgData from "@/components/imgData";

export default defineComponent({
    name: "MainPage",
    data() {
        return {
            imgdata: imgData,
            one: 1,
            number: "",
        };
    },
    methods: {
        ones() {
            this.number += "1";
        },
        //     // result() {
        //     //     console.log(this.one);
        //     // },
    },
});
</script>

<style scoped="lang">
.backgroundDiv {
    margin-top: -60px;
}
.backImg {
    background-image: url("../assets/mainPage.png");
    background-repeat: no-repeat;
    background-size: cover;
    background-position: center;
    width: 100%;
    height: 820px;
}
@media screen and (max-width: 768px) {
    .imgDataDisplay {
        margin-left: auto;
        margin-right: auto;
    }
    .btnBackDisplay {
        margin-left: auto;
        margin-right: auto;
        margin-top: 20px;
        width: 25rem;
    }
    .download {
        margin-left: auto;
        margin-right: auto;
        margin-top: 20px;
        width: 25rem;
    }
}
@media screen and (min-width: 769px) {
    .imgDataDisplay {
        /* display: flex;
        justify-content: end; */
        margin-left: 920px;
        margin-bottom: -50px;
    }
    .btnBackDisplay {
        margin-left: 800px;
        margin-top: 100px;
        width: 25rem;
    }
    .download {
        margin-left: 720px;
        margin-top: 100px;
        width: 25rem;
    }
}
</style>
