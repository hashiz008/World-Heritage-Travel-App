...
function initAutocomplete() {
  //マップの初期設定です。
  const map = new windogoogle.maps.Map(document.getElementById("map"), {
    center: { lat: -33.8688, lng: 151.2195 },
    zoom: 13,
    mapTypeId: "roadmap",
  });
  const input = document.getElementById("pac-input");
  const searchBox = new windogoogle.maps.places.SearchBox(input);
 ...
 ////[https://developers.google.com/maps/documentation/javascript/reference/places-widget#SearchBox]
  ...
  ////https://developers.google.com/maps/documentation/javascript/reference/places-widget#Autocomplete.place_changed
    ...
    const bounds = new windogoogle.maps.LatLngBounds();
    ...
    places.forEach((place) => {
      ...
      const icon = {
        ...
        size: new windogoogle.maps.Size(71, 71),
        origin: new windogoogle.maps.Point(0, 0),
        anchor: new windogoogle.maps.Point(17, 34),
        ////"Point"クラスはマーカーのラベルなどの位置を決めるインスタンスメソッド。
        ////https://lab.syncer.jp/Web/API/Google_Maps/JavaScript/Point/

        scaledSize: new windogoogle.maps.Size(25, 25),
      };
      // Create a marker for each place.
      markers.push(
        new windogoogle.maps.Marker({
          map,
          icon,
          title: place.name,
