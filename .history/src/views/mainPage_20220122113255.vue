    ...
    <div class="backgroundDiv vw-100">
        <div class="backImg">
            <div class="inStyle">
                ...
                >
                    <h1  class="resultNumber text-white pr-40 pt-1 text-2xl font-bold">
                        Number:{{ number }}
                    </h1>
                </div>
                <div
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            hrefs: "" as string,
            down: "" as string,
            myLatLng: { lat: -32.397, lng: 151.644 } as {
                lat: number;
                lng: number;
            },
        };
...
function initMap() {
    var opts = {
        zoom: 15,
        center: new window.google.maps.LatLng(35.709984, 139.810703),
    };
    let map = new window.google.maps.Map(document.getElementById("map"), opts);
    new window.google.maps.Marker({
        position: new window.google.maps.LatLng(35.709984, 139.810703),
        map,
    });
    // console.log(marker)
    let imageNums: any = document.getElementById("imgNums")
    let btnNums = Number(imageNums.querySelector("h1").textContent.slice(8));
    // console.log(imgData)
    for (let i in imgData) {
        if (imgData[i].num == btnNums) console.log(marker)
    }
}
</script>
