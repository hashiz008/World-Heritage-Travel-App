        ...
        <div class="backImg">
            <div class="inStyle">
                ...
                <div class="gooleMapDiv">
                 ...
                 <!-- <div class="Homebtn"> -->
                <router-link :to="{params'/'}"></router-link>
            <!-- </div> -->
                <div
                    id="imgNums"
...
export default defineComponent({
    ...
    mounted() {
        const map:{center:number,zoom:number} = new window.google.maps.Map(this.$refs.map, {
    ...
    methods: {
        result() {
            ...
            if (placeName == "") {
                ...
                if (window.confirm("0~9以内の範囲以内で選択してください！")) {
                    console.log(this.country="フランス")
                }
            } else {
...
function initMap():void {
    ...
    let btnNums:number | string = Number(imageNums.querySelector("h1").textContent.slice(8));
    for (let i in imgData) {
        if (imgData[i].num == btnNums) {
            let opts:{zoom:number,center:number} = {
                zoom: 12,
                center: new window.google.maps.LatLng(imgData[i].lat,imgData[i].lng),
            };
